...
def get_chosen_doc(num_doc, lendoc):
    random.seed()
    duplicated = 0
    result = []
    while len(result) < num_doc:
        if len(result) % 100 == 0:
            print len(result)
        a = random.randint(1, lendoc)
...
def write_doc_doc(con, cur, gamma_file, num_doc):
    ...
    chosen_doc = get_chosen_doc(num_doc, len(docs))
    for a in range(len(chosen_doc)):
        if a % 100 == 0:
            print a
        doc_by_doc = {}
        for b in range(a, len(chosen_doc)):
            score = get_doc_score(docs[chosen_doc[a] - 1], docs[chosen_doc[b] - 1])
            if score == 0:
                continue
...
def write_docs(con, cur, docs_file, chosen_docs):
    cur.execute('CREATE TABLE docs (id INTEGER PRIMARY KEY, title VARCHAR(100))')
    con.commit()

    for i in chosen_docs:
        line = linecache.getline(docs_file, i)
        psudotitle = line.split('", "')[3].split('": "')[1][0:50]
        cur.execute('INSERT INTO docs (id, title) VALUES(?, ?)', [str(i), psudotitle])
...
if (__name__ == '__main__'):
    ...
    chosen_doc = write_doc_doc(con, cur, gamma_file, num_doc)

    print "writing docs to db..."
    write_docs(con, cur, doc_file, chosen_doc)
